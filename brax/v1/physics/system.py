...
from brax.v1.physics.base import Info, P, QP, validate_config, vec_to_arr
...
class System:
  ...
  def info(self, qp: QP) -> Info:
    ...
    print(f"\tqp: {qp} // {jnp.isnan(qp.pos).sum()} // {jnp.isnan(qp.rot).sum()} // {jnp.isnan(vel).sum()} // {jnp.isnan(qp.ang).sum()}")
    qqq
